...
int move_step(char *seq1, char *seq2)
{
  ...
  N = strlen(seq2);
  printf("move\n");
  printf("seq1=%d, SEQX=%d", M, strlen(SEQX));
  x = malloc(sizeof(char) * (M+2)); /* +2: leading blank, and trailing \0 */
  y = malloc(sizeof(char) * (N+2));
  strcpy(x+1, seq1);		    /* x_1..x_M now defined; x_0 undefined */
  strcpy(y+1, seq2);		    /* y_1..y_N now defined; y_0 undefined */
